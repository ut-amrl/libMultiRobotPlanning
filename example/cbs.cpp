...
class Environment {
 public:
  ...
  int admissibleHeuristic(const State& s) {
    // std::cout << "H: " <<  s << " " << m_heuristic[m_agentIdx][s.x + dimx_ *
    // s.y] << std::endl;
    // return m_heuristic[m_agentIdx][s.x + dimx_ * s.y];
  ...
  void computeHeuristic()
  {
    ...
    public:
      ...
      void onDiscover(
        ...
        int gScore)
      {
        (*m_heuristic)[s.x + dimx_ * s.y] = gScore;
      }
    ...
    private:
        ...
        const Location& s)
      {
        return    s.x >= 0
               ...
               && s.y >= 0
               && s.y < dimy_
               && obstacles_.find(Location(s.x, s.y)) == obstacles_.end();
      }
    ...
    m_heuristic.resize(goals_.size());

    std::vector< Neighbor<State, Action, int> > neighbors;

    for (size_t i = 0; i < goals_.size(); ++i) {
      m_heuristic[i].assign(dimx_ * dimy_, std::numeric_limits<int>::max());
      HeuristicEnvironment henv(dimx_, dimy_, obstacles_, &m_heuristic[i]);
      AStar<Location, Action, int, HeuristicEnvironment> astar(henv);
      PlanResult<Location, Action, int> dummy;
      astar.search(goals_[i], dummy);
      m_heuristic[i][goals_[i].x + dimx_ * goals_[i].y] = 0;
    }
  }
#endif
 private:
