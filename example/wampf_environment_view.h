...
//  virtual bool Overlaps(const EnvironmentView& other) const = 0;
  ...
  virtual void setLowLevelContext(size_t agentIdx, const Constraints* constraints) = 0;
  ...
  virtual void createConstraintsFromConflict(
          const Conflict& conflict, std::map<size_t, Constraints>& constraints) = 0;

  virtual void onExpandHighLevelNode(int /*cost*/) = 0;

  virtual void onExpandLowLevelNode(const CBSState& /*s*/, int /*fScore*/, int /*gScore*/) = 0;
};
