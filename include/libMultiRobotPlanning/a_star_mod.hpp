...
class AStarMod {
 public:
  AStarMod(Environment* environment) : m_env(environment) {}
  AStarMod(const AStarMod&) = default;
  AStarMod(AStarMod&&) = default;

  AStarMod& operator=(const AStarMod&) = default;
  AStarMod& operator=(AStarMod&&) = default;
  bool search(const State& startState,
              PlanResult<State, Action, Cost>& solution, int time_offset,
              Cost initialCost = 0) {
    ...
    while (!openSet.empty()) {
      ...
      if (m_env->isSolution(current.state)) {
        ...
        m_env->FixPadTime(solution.states);
        return true;
      }
